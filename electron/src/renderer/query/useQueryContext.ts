 ...
 *
 * 查询控制台按标签自带连接（同 FX 版每个查询页各有一套 PathSelector）：
 * 库 / 模式 / 表候选一律取自「当前查询标签所属连接」，不跟着活动连接跑，
 * 否则先开 A、B 再从 A 的脚本打开控制台，界面会显示成 B 的连接与库。
 */
export function useQueryContext(deps: QueryContextDeps) {
  ...
  /*
   * 控制台自己的会话与根节点。查询页只认自己的连接：连接已关就是没有会话，
   * 不能退回活动会话，否则会拿别的连接的库 / 表当这个控制台的上下文。
   */
  const consoleSession = tabConnection ? openSessions[tabConnection] : session;
  const consoleRoots = tabConnection ? rootsByConnection[tabConnection] ?? [] : roots;
  ...
  function consoleConnection(): string | undefined {
    ...
    return tabConnection ?? activeTab.script?.connection ?? lastSessionRef.current.name;
  }
  ...
  suggestionContextRef.current = (() => {
     ...
     */
    const sameConnection = Boolean(consoleSession && (!tabConnection || tabConnection === consoleSession.name));
    /*
  ...
  /* 把控制台所属连接的根节点作为「数据库」候选，并给查询页一个默认上下文 */
  useEffect(() => {
    setCatalogOptions(consoleRoots);

    if (consoleRoots.length === 0)
      return;

    setTabs(previous => previous.map(tab =>
      tab.kind === "query" && !tab.path.catalog
        /* 只给「没有连接」或「就属于当前控制台连接」的查询页补默认库：
           别的连接的查询页再拿这条连接的库去补，执行时就会拿错库 */
        && (!tab.path.connection || tab.path.connection === consoleSession?.name)
        ? { ...tab, path: { ...tab.path, catalog: consoleRoots[0].label } }
        : tab));
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const children = await loadChildren(consoleSession.sessionId, catalogNode);
      ...
      if (schemas.length === 0 && container) {
        const tables = await loadChildren(consoleSession.sessionId, container);
  ...
  useEffect(() => {
    ...
    if (!consoleSession || !activeSchema)
      return;
    ...
    void (async () => {
      ...
      const children = await loadChildren(consoleSession.sessionId, schemaNode);
  ...
  }, [consoleSession, activeSchema, schemaOptions]);
   ...
   *
   * 默认库取活动连接的根节点（新建查询跟着活动连接 / 对象树选中走），不是控制台的库。
   */
  function selectionContext(): { connection?: string; catalog?: string; schema?: string } {
    const context = selectionContextHelper(activeNode, roots, treeRoot, treeChildren);
    const activeName = session?.name;

    if (activeName && context.connection && context.connection !== activeName)
      return { connection: activeName, catalog: roots[0]?.label };
